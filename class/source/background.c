...
int background_functions(
  ...
  if(pba->has_iDMDEg == _TRUE_){
    ...
    pvecback[pba->index_bg_rho_g] = rho_g0*pow(a_rel,-4)
                                    +rho_de0*pba->m_iDE*pba->epsilon_iDE/(3.*w_fld_eff-1.)
                                            *(1.-pow(a_rel,-3.*w_fld_eff+1.))*pow(a_rel,-4.);
  }
  else{
  ...
  if (pba->has_cdm == _TRUE_) {
    ...
    if(pba->has_iDMDEg == _TRUE_){
      ...
      double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
      double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
      pvecback[pba->index_bg_rho_cdm] = rho_dm0*pow(a_rel,-3.)
                                        +rho_de0*pba->m_iDE*(1.-pba->epsilon_iDE)/(3.*w_fld_eff)
                                                *(1.-pow(a_rel,-3.*w_fld_eff))*pow(a_rel,-3.);
    }
    else{
...
int background_init(
  ...
  /* BEGIN MODIFICATION ML */
  if (pba->has_fld == _TRUE_ && pba->has_iDMDEg == _FALSE_) {
  /* END MODIFICATION ML */
