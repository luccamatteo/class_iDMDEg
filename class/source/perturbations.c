      ...
      if (pba->has_fld == _TRUE_) {
        ...
        if (pba->use_ppf == _FALSE_) {
          ...
          if(pba->has_iDMDEg == _TRUE_){
            ppw->pv->y[ppw->pv->index_pt_delta_fld] = (1+w_fld-pba->m_iDE*2.)
                                                      *(1+w_fld+pba->m_iDE/3.)
                                                      /(12*w_fld*w_fld-2*w_fld-3*w_fld*pba->m_iDE+7*pba->m_iDE-14)
                                                      *(-2.*ppw->pv->y[ppw->pv->index_pt_delta_g]/(1+1./3.));
            ppw->pv->y[ppw->pv->index_pt_theta_fld] = k*tau*(1+w_fld+pba->m_iDE/3.)
                                                      /(12*w_fld*w_fld-2*w_fld-3*w_fld*pba->m_iDE+7*pba->m_iDE-14)
                                                      *(-2.*ppw->pv->y[ppw->pv->index_pt_delta_g]/(1+1./3.));
          }
          else{
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
      ...
      if(pba->has_iDMDEg == _TRUE_){
        dy[pv->index_pt_delta_g] = -4./3.*(theta_g+metric_continuity)
                                   +pba->epsilon_iDE*pba->m_iDE*pvecback[pba->index_bg_rho_fld]/pvecback[pba->index_bg_rho_g]
                                      *(a_prime_over_a*(y[pv->index_pt_delta_fld]-y[pv->index_pt_delta_g])+metric_continuity/3.);
      }
      else{
        dy[pv->index_pt_delta_g] = -4./3.*(theta_g+metric_continuity);
      }
      ...
      if (ppt->gauge == synchronous) {
        ...
        if(pba->has_iDMDEg == _TRUE_){
          dy[pv->index_pt_delta_cdm] = -metric_continuity
                                          +(1.-pba->epsilon_iDE)*pba->m_iDE*pvecback[pba->index_bg_rho_fld]/pvecback[pba->index_bg_rho_cdm]
                                              *(a_prime_over_a*(y[pv->index_pt_delta_fld]-y[pv->index_pt_delta_cdm])+metric_continuity/3.);
        }
        else{
          dy[pv->index_pt_delta_cdm] = -metric_continuity;
        }
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        if(pba->has_iDMDEg == _TRUE_){
          dy[pv->index_pt_delta_fld] = -(1+w_fld)*(y[pv->index_pt_theta_fld]+metric_continuity)
                                       -3.*a_prime_over_a*(1.-w_fld)*(y[pv->index_pt_delta_fld]
                                                                        +a_prime_over_a*y[pv->index_pt_theta_fld]/k2
                                                                              *(3.*(1.+w_fld)+pba->m_iDE))
                                        -pba->m_iDE*metric_continuity/3.;
        }
        else{
        ...
        if(pba->has_iDMDEg == _TRUE_){
          dy[pv->index_pt_theta_fld] = 2*a_prime_over_a*y[pv->index_pt_theta_fld]*
                                              (1.+pba->m_iDE/(1+w_fld)*(1.
                                                      -(1.-pba->epsilon_iDE)*y[pv->index_pt_theta_cdm]/2./y[pv->index_pt_theta_fld]
                                                      -4.*pba->epsilon_iDE/3.*theta_g/2./y[pv->index_pt_theta_fld])
                                              )
