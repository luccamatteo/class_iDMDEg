...
int thermodynamics_at_z(
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    if(pba->has_iDMDEg == _TRUE_){
      double rho_g0 = pba->Omega0_g*pow(pba->H0,2.);
      double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
      double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
      pvecthermo[pth->index_th_Tb] = pba->T_cmb*(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                        *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.);
    }
    else{
    ...
    if(pba->has_iDMDEg == _TRUE_){
      ...
      double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
      double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
      pvecthermo[pth->index_th_wb] = _k_B_/( _c_*_c_*_m_H_)*(1.+(1./_not4_-1.)*pth->YHe+x0*(1.-pth->YHe))
                                               *pba->T_cmb*(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                        *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.);
    }
    else{
...
int thermodynamics_helium_from_bbn(
  ...
  /* BEGIN MODIFICATION ML */
  if(pba->has_iDMDEg == _TRUE_){
    /** Here we need to recast the condition T_g = T0*(1+_z_bbn) = 0.1 MeV in the context of the iDE model.
        To solve Eq. (13) of the paper for z_bbn we assume that w_x appox -1, m is small, w_g approx 1/3, and
        that therefore at early times we can neglect the z dependence of the iDE term.
        Then we have that T_g = T_0*(1+zz_bbn)*(1+eps*m*rho_x0/3/(w_eff-w_g)/rho_g0)^(1/4)
        which can be solved for to z_bbn to have
        z_bbn = (T_g/T_0)*(1+eps*m*rho_x0/3/(w_eff-w_g)/rho_g0)^(-1/4)-1 **/
    double rho_g0 = pba->Omega0_g*pow(pba->H0,2.);
    double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
    double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
    z_bbn = (0.1/(8.6173e-11*pba->T_cmb))*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0, -1./4.)-1.0;
  }
  else{
  ...
  for (i=preio->rt_size-1; i >0 ; i--) {
    ...
    if(pba->has_iDMDEg == _TRUE_){
      ...
      dTdz=2./(1+z)*preio->reionization_table[i*preio->re_size+preio->index_re_Tb]
        ...
        (pba->T_cmb*(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                      *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.)
                    -preio->reionization_table[i*preio->re_size+preio->index_re_Tb])/pvecback[pba->index_bg_H];
    }
    else{
      dTdz=2./(1+z)*preio->reionization_table[i*preio->re_size+preio->index_re_Tb]
        -2.*mu/_m_e_*4.*pvecback[pba->index_bg_rho_g]/3./pvecback[pba->index_bg_rho_b]*opacity*
...
int thermodynamics_recombination_with_hyrec(
  ...
  /* BEGIN MODIFICATION ML */
  class_test(pba->has_iDMDEg == _TRUE_,
             pth->error_message,
             "The iDE model is not accounted for in HYREC. Choose RECFAST.");
  /* END MODIFICATION ML */
...
int thermodynamics_recombination_with_recfast(
  ...
  if(pba->has_iDMDEg == _TRUE_){
    ...
    double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
    double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
    y[2] = preco->Tnow*(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                      *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.);
  }
  else{
    y[2] = preco->Tnow*(1.+z);
  ...
  for(i=0; i <Nz; i++) {
    ...
    /** - --> second approximation: first Helium recombination (analytic approximation) */
      ...
      if(pba->has_iDMDEg == _TRUE_){
        ...
        double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
        double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
        rhs = exp(1.5*log(preco->CR*preco->Tnow/(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                                *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.)
                           )-preco->CB1_He2/(preco->Tnow*(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                                *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.)
                           ))/preco->Nnow;
      }
      else{
    ...
    /** - --> fourth approximation: second Helium recombination starts (analytic approximation) */
      ...
      if(pba->has_iDMDEg == _TRUE_){
        ...
        double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
        double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
        rhs = 4.*exp(1.5*log(preco->CR*preco->Tnow/(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                                *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.)
                           )-preco->CB1_He1/(preco->Tnow*(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                                *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.)
                           ))/preco->Nnow;
      }
      else{
    ...
    /** - --> fifth approximation: second Helium recombination (full
      ...
      if(pba->has_iDMDEg == _TRUE_){
        ...
        rhs = exp(1.5*log(preco->CR*preco->Tnow/(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                                *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.)
                           )-preco->CB1/(preco->Tnow*(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                                *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.)
                           ))/preco->Nnow;
      }
      else{
...
int thermodynamics_derivs_with_recfast(
  ...
  if(pba->has_iDMDEg == _TRUE_){
    ...
    double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
    double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
    Trad = preco->Tnow*(1.+z)*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                       *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.);
  }
  else{
    Trad = preco->Tnow * (1.+z);
  ...
  if (timeTh < preco->H_frac*timeH) {
    ...
    if(pba->has_iDMDEg == _TRUE_){
      ...
      double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
      double w_fld_eff = pba->w0_fld+pba->m_iDE/3.;
      dy[2] = preco->Tnow*pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                *(1.-pow(1.+z, 3.*w_fld_eff-1.)), 1./4.)
                  + epsilon*((1.+preco->fHe)/(1.+preco->fHe+x))*((dy[0]+preco->fHe*dy[1])/x)
      ...
      for (i=0; i<ppr->thermo_Nz2_idm_dr+ppr->thermo_Nz1_idm_dr-1; i++){
        ...
        if(pba->has_iDMDEg == _TRUE_){
          ...
          pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_Tb]=pba->T_cmb*(1.+pth->z_table[index_th])
                                                   *pow(1.+pba->epsilon_iDE*pba->m_iDE*rho_de0/(3.*w_fld_eff-1.)/rho_g0
                                                            *(1.-pow(1.+pth->z_table[index_th], 3.*w_fld_eff-1.)), 1./4.);
          pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_wb]=_k_B_/(_c_*_c_*_m_H_)*(1.+(1./_not4_-1.)*pth->YHe+x0*(1.-pth->YHe))*
